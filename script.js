// script.js (Versão 20.0 - Correções Aplicadas)
console.log('Script.js loaded - Version 20.0');
...
function abrirModalComPreco(button) {
  ...
  if (sabores && sabores.length > 0) {
    // A frase "Selecione os Sabores e a Quantidade:" já está no HTML, não precisa ser adicionada aqui.
...
function updateCartCounter() {
  ...
  const totalProdutos = carrinho.reduce((acc, item) => acc + (item.quantidade || 1), 0);
  const contadorElement = document.getElementById('contador');
  if (contadorElement) {
    contadorElement.innerText = totalProdutos;
  }
}
